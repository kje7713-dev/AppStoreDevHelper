...
import { saveAudit, listAuditsForApp } from "@/lib/audit-store"
import { generateMockAudit } from "@appops/core/release-audit"
import { AuditInputSchema } from "@appops/schemas/release-schema"
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ appId: string }> }) {
  ...
  const audits = listAuditsForApp(appId)
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ appId: string }> }) {
  ...
  const result = AuditInputSchema.safeParse(body)
  if (!result.success) {
