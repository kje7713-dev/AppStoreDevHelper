...
export default function AuditResultsByIdPage() {
  ...
  useEffect(() => {
    fetch(`/api/apps/${appId}/release/audit/${auditId}`)
      .then((r) => {
    ...
    <main className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-4xl mx-auto px-6 py-12">
        <div className="flex items-start justify-between mb-8">
          <div>
            <Link href={`/apps/${appId}/audit`} className="text-gray-400 text-sm hover:text-white mb-2 block">
              ← Run New Audit
            </Link>
              ...
              {audit.blockingIssues.map((issue, i) => (
                <div key={i} className="p-4 rounded-xl bg-gray-900 border border-gray-800">
                  <div className="flex items-start gap-3">
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${SEVERITY_COLORS[issue.severity]} shrink-0 mt-0.5`}
                    >
                      {issue.severity}
                    </span>
                    <div>
                ...
                <div key={i} className="p-5 rounded-xl bg-gray-900 border border-gray-800">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${
                        SEVERITY_COLORS[task.priority] ?? "bg-gray-800 text-gray-300"
                      }`}
                    >
                      {task.priority}
                    </span>
                  ...
                  {task.labels && task.labels.length > 0 && (
                    <div className="flex gap-2 mt-3">
                      {task.labels.map((l, j) => (
                        <span key={j} className="text-xs px-2 py-0.5 rounded bg-gray-800 text-gray-400">
                          {l}
                        </span>
                      ))}
                    </div>
            ...
            >
              <div
                className={`w-4 h-4 rounded border shrink-0 mt-0.5 flex items-center justify-center ${
                  checked.has(i) ? "bg-indigo-600 border-indigo-600" : "border-gray-600"
                }`}
              >
                {checked.has(i) && (
              ...
              <span className={`text-sm ${checked.has(i) ? "line-through text-gray-500" : "text-gray-300"}`}>
                {item}
              </span>
            </button>
          </li>
